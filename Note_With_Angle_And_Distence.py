...
def calculat_angles(fov, x, y, h, w ,frame):
    X_Angle = (frame.shape[0]/fov[0])*np.abs((frame.shape[0]/2) - (x+(int(w/2))))
    Y_Angle = ((frame.shape[1]/fov[1])*np.abs((frame.shape[1]/2) - (y+(int(h/2)))) + camera_angle)
        ...
        if biggest_child_contour_index != -1:
            ...
            if (cv2.contourArea(outer_contour) > 3600) and (cv2.contourArea(inner_contour) > 3600):
                ...
                if (abs(outer_aspect_ratio - inner_aspect_ratio) < 0.2) and (math.dist(outer_center, inner_center) < 8):
                    ...
                    Angles = calculat_angles(field_of_view, x, y, h, w ,img)
                    dist = calculat_distence(Angles,camera_hight)
